document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function loadPersonnelTable(filterDept = '', filterPoste = '', filterTypeContrat='') {
  ...
  tr.addEventListener('click', function(e) {
    ...
    document.querySelectorAll('.edit-emp').forEach(btn => {
      btn.onclick = async (e) => {
        ...
        addEmployeeForm.elements.date_naissance.value = emp.date_naissance;
        ...
        addEmployeeForm.elements.date_embauche.value = emp.date_embauche;
    ...
    document.querySelectorAll('.delete-emp').forEach(btn => {
      btn.onclick = async (e) => {
        ...
        if (result.success) {
          alert("Employé supprimé avec succes !");
...
if (addEmployeeForm.dataset.editing !== "true" && existe) {
  ...
  return;
    ...
    const employee = {
      ...
      date_naissance: addEmployeeForm.elements.date_naissance.value,
      ...
      date_embauche: addEmployeeForm.elements.date_embauche.value
    };
...
document.getElementById('filter-departement').addEventListener('change', async function() {
  await loadPersonnelTable(this.value, document.getElementById('filter-poste').value);
});
document.getElementById('filter-poste').addEventListener('change', async function() {
  await loadPersonnelTable(document.getElementById('filter-departement').value, this.value);
});
...
function afficherModaleEmploye(emp, posteIntitule) {
  const html = `
    ...
    <div><b>Date de naissance :</b> ${emp.date_naissance}</div>
    ...
    <div><b>Date d'embauche :</b> ${emp.date_embauche}</div>
  `;
...
function calculerAge(dateNaissance) {
  ...
  // Empêche les âges négatifs si la date est dans le futur
  return age < 0 ? 0 : age;
}
